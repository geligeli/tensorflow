...
class MCTS {
  ...
  std::chrono::milliseconds allowedComputationTime =
 ...
 public:
   ...
   * instance
   */
  MCTS(const T& rootData, Backpropagation<T>* backprop,
       TerminationCheck<T>* termination, Scoring<T>* scoring)
      : backprop(backprop),
  ...
  void setTime(int time) {
    this->allowedComputationTime = std::chrono::milliseconds(time);
  }
 ...
 private:
  ...
  std::shared_ptr<Node<T, A, E>> select(const Node<T, A, E>& node) {
    ...
    for (auto& n : children) {
      float score =
          n->getAvgScore() +
          C * (float)sqrt(log(node.getNumVisits()) / n->getNumVisits());
  ...
  std::shared_ptr<Node<T, A, E>> expandNext(
    ...
    auto newNode = std::make_shared<Node<T, A, E>>(++currentNodeID,
                                                   expandedData, node, action);
